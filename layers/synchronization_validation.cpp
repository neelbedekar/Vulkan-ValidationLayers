...
// resolves the pendinging state. Suitable for processing Global memory barriers, or Subpass Barriers when the "final" barrier
    ...
    if (IsRead(usage_bit)) {
        ...
        if (is_raw_hazard) {
            ...
            const bool usage_is_ordered =
                (input_attachment_ordering && usage_is_input_attachment) || (0 != (usage_bit & ordering.exec_scope));
            if (usage_is_ordered) {
